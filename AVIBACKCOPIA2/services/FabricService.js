  ...
  async ejecutarPrediccionIncremental({ enviarAPowerBI = true } = {}) {
    ...
    const payloadProgramas = programasAActualizar.map(prog => {
      ...
      const desertores = aprendices.filter(a =>
        ['desertor', 'inasistencia', 'retiro voluntario', 'retiro'].includes(a.estado.toLowerCase())
      ).length;

      const exitosos = aprendices.filter(a =>
        ['certificado', 'en formacion', 'activo'].includes(a.estado.toLowerCase())
      ).length;
      ...
      // Extraer barrios únicos e institución más frecuente
      const barrios = new Set();
      const instituciones = {};
      let institucion_principal = "";
      let maxInstCount = 0;

      for (const rec of recomendaciones) {
          if (rec.reporte && rec.reporte.aspirante) {
              const asp = rec.reporte.aspirante;
              if (asp.barrio) barrios.add(asp.barrio);
              if (asp.institucion) {
                  instituciones[asp.institucion] = (instituciones[asp.institucion] || 0) + 1;
                  if (instituciones[asp.institucion] > maxInstCount) {
                      maxInstCount = instituciones[asp.institucion];
                      institucion_principal = asp.institucion;
                  }
              }
          }
      }
      
      const diversidad_barrios = barrios.size;
      ...
      return {
        ...
        diversidad_barrios:        diversidad_barrios,
        institucion_principal:     institucion_principal,
